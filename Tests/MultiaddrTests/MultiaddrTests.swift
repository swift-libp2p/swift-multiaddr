...
final class MultiaddrTests: XCTestCase {
    ...
    func testCreateMultiaddrFromString_FailsWithInvalidStrings() throws {
        ...
        for addr in addresses {
            XCTAssertThrowsError(
                try Multiaddr(addr),
                "Invalid string address `\(addr)` resulted in Multiaddr creation."
            )
        }
    }
    ...
    func testIPv6FromString() throws {
        ...
        XCTAssertEqual(
            try IPv6.data(for: addy.address!).asString(base: .base16Upper),
            "ABCD0000000100020003000400050006"
        )
    }
    ...
    func testValidMutliaddr() throws {
        ...
        for address in validAddresses {
            do {
                ...
                XCTAssertEqual(
                    addy.description,
                    address.hasSuffix("/") ? String(address.dropLast()) : address,
    ...
    func testStringAddressToBinaryPacked() throws {
        ...
        XCTAssertEqual(
            try Multiaddr("/onion/aaimaq4ygg2iegci:80").binaryPacked().toHexString(),
    ...
    func testBinaryPackedToStrings() throws {
        ...
        XCTAssertEqual(
            try Multiaddr(Data(hex: "bc030010c0439831b48218480050")).description,
